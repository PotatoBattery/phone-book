<template>
    <Head title="Список избранных контактов" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Список избранных контактов</h2>
            <input v-model="searchQuery" type="text" id="searchLine" placeholder="Поиск по избранным контактам" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none">
        </template>
                        ...
                        <table class="w-full text-center">
                            ...
                            <tr class="bg-white border-b" v-if="isEmpty">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center" colspan="5">
                                    Избранные контакты в Вашей телефонной книге отсутствуют.
                                </td>
                            </tr>
...
export default {
    ...
    methods: {
        getFavoriteContacts(){
            window.axios.get(`/api/v1/phone-book/favorite/contacts?searchQuery=${this.searchQuery}`)
                .then(response => {
